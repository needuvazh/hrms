...
public class LeaveApplicationService implements LeaveModuleApi {
                    ...
                    return leaveRepository.findLeaveBalance(tenantId, command.employeeId(), command.leaveTypeId(), leaveYear)
                            ...
                            .flatMap(balance -> {
                                ...
                                return leaveRepository.saveLeaveRequest(draft)
                                        .flatMap(saved -> workflowModuleApi.startWorkflow(new StartWorkflowCommand(
                                                        LEAVE_WORKFLOW_KEY,
                                                        "LEAVE_REQUEST",
                                                        saved.id().toString(),
                                                        command.requestedBy().trim(),
                                                        command.reason()))
                                                .flatMap(workflow -> leaveRepository.updateLeaveRequest(new LeaveRequest(
                                                        saved.id(),
                                                        saved.tenantId(),
                                                        saved.employeeId(),
                                                        saved.leaveTypeId(),
    ...
    public Flux<LeaveBalanceView> balances(UUID employeeId, int leaveYear) {
        if (employeeId == null) {
            return Flux.error(new HrmsException(HttpStatus.BAD_REQUEST, "EMPLOYEE_REQUIRED", "Employee id is required"));
        }
    ...
    private Mono<LeaveRequest> updateLeaveDecision(LeaveRequest existing, ReviewLeaveCommand command) {
        Instant now = Instant.now();
        LeaveRequest updated = existing.decide(command.decision(), command.reviewer().trim(), now);

        Mono<LeaveRequest> decisionUpdate = leaveRepository.updateLeaveRequest(updated);
    ...
    private void validateLeaveType(DefineLeaveTypeCommand command) {
    ...
    private void validateInitializeBalance(InitializeLeaveBalanceCommand command) {
    ...
    private void validateApply(ApplyLeaveCommand command) {
        if (command.employeeId() == null) {
            throw new HrmsException(HttpStatus.BAD_REQUEST, "EMPLOYEE_REQUIRED", "Employee id is required");
        }
        if (command.leaveTypeId() == null) {
    ...
    private void validateReview(ReviewLeaveCommand command) {
    ...
    private LeaveTypeView toLeaveTypeView(LeaveType leaveType) {
        return new LeaveTypeView(
                leaveType.id(),
                leaveType.tenantId(),
                leaveType.leaveCode(),
                leaveType.name(),
    ...
    private LeaveBalanceView toLeaveBalanceView(LeaveBalance leaveBalance) {
    ...
    private LeaveRequestView toLeaveRequestView(LeaveRequest leaveRequest) {
    ...
    private String leaveRequestedPayload(LeaveRequest leaveRequest) {
