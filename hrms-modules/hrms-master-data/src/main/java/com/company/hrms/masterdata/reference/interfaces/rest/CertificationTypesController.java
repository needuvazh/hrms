...
@RestController
@Validated
@RequestMapping("/api/masters/reference/certification-types")
@Tag(name = "Reference Certification Types", description = "Certification type master APIs")
public class CertificationTypesController {

    private final ReferenceControllerService service;

    public CertificationTypesController(@Qualifier("certificationTypesService") ReferenceControllerService service) {
        this.service = service;
    }

    @PostMapping
    public Mono<CertificationTypesViewDto> create(@Valid @RequestBody CertificationTypesUpsertRequestDto request) {
        return service.create(request.toReferenceRequest()).map(ReferenceViewDtoMapper::toCertificationTypes);
    }

    @PutMapping("/{id}")
    public Mono<CertificationTypesViewDto> update(@PathVariable("id") UUID id, @Valid @RequestBody CertificationTypesUpsertRequestDto request) {
        return service.update(id, request.toReferenceRequest()).map(ReferenceViewDtoMapper::toCertificationTypes);
    }

    @GetMapping("/{id}")
    public Mono<CertificationTypesViewDto> getById(@PathVariable("id") UUID id) {
        return service.getById(id).map(ReferenceViewDtoMapper::toCertificationTypes);
    }

    @GetMapping
    public Mono<PagedResult<CertificationTypesViewDto>> list(
        ...
        return service.list(new ReferenceSearchQuery(q, active, page, size, sort, skillCategoryId, all))
                .map(result -> new PagedResult<>(
                        result.items().stream().map(ReferenceViewDtoMapper::toCertificationTypes).toList(),
                        result.page(),
                        result.size(),
