...
@RestController
@Validated
@RequestMapping("/api/masters/reference/countries")
@Tag(name = "Reference Countries", description = "Country master APIs")
public class CountriesController {

    private final ReferenceControllerService service;

    public CountriesController(@Qualifier("countriesService") ReferenceControllerService service) {
        this.service = service;
    }

    @PostMapping
    public Mono<CountriesViewDto> create(@Valid @RequestBody CountriesUpsertRequestDto request) {
        return service.create(request.toReferenceRequest()).map(ReferenceViewDtoMapper::toCountries);
    }

    @PutMapping("/{id}")
    public Mono<CountriesViewDto> update(@PathVariable("id") UUID id, @Valid @RequestBody CountriesUpsertRequestDto request) {
        return service.update(id, request.toReferenceRequest()).map(ReferenceViewDtoMapper::toCountries);
    }

    @GetMapping("/{id}")
    public Mono<CountriesViewDto> getById(@PathVariable("id") UUID id) {
        return service.getById(id).map(ReferenceViewDtoMapper::toCountries);
    }

    @GetMapping
    public Mono<PagedResult<CountriesViewDto>> list(
        ...
        return service.list(new ReferenceSearchQuery(q, active, page, size, sort, skillCategoryId, all))
                .map(result -> new PagedResult<>(
                        result.items().stream().map(ReferenceViewDtoMapper::toCountries).toList(),
                        result.page(),
                        result.size(),
