...
@RestController
@Validated
@RequestMapping("/api/masters/reference/currencies")
@Tag(name = "Reference Currencies", description = "Currency master APIs")
public class CurrenciesController {

    private final ReferenceControllerService service;

    public CurrenciesController(@Qualifier("currenciesService") ReferenceControllerService service) {
        this.service = service;
    }

    @PostMapping
    public Mono<CurrenciesViewDto> create(@Valid @RequestBody CurrenciesUpsertRequestDto request) {
        return service.create(request.toReferenceRequest()).map(ReferenceViewDtoMapper::toCurrencies);
    }

    @PutMapping("/{id}")
    public Mono<CurrenciesViewDto> update(@PathVariable("id") UUID id, @Valid @RequestBody CurrenciesUpsertRequestDto request) {
        return service.update(id, request.toReferenceRequest()).map(ReferenceViewDtoMapper::toCurrencies);
    }

    @PutMapping
    public Mono<CurrenciesViewDto> updateByQueryId(
            @RequestParam("id") UUID id,
            @Valid @RequestBody CurrenciesUpsertRequestDto request
    ) {
        return service.update(id, request.toReferenceRequest()).map(ReferenceViewDtoMapper::toCurrencies);
    }

    @GetMapping("/{id}")
    public Mono<CurrenciesViewDto> getById(@PathVariable("id") UUID id) {
        return service.getById(id).map(ReferenceViewDtoMapper::toCurrencies);
    }

    @GetMapping
    public Mono<PagedResult<CurrenciesViewDto>> list(
        ...
        return service.list(new ReferenceSearchQuery(q, active, page, size, sort, skillCategoryId, all))
                .map(result -> new PagedResult<>(
                        result.items().stream().map(ReferenceViewDtoMapper::toCurrencies).toList(),
                        result.page(),
                        result.size(),
