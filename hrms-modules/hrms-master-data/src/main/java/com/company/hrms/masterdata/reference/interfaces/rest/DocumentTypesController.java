...
@RestController
@Validated
@RequestMapping("/api/masters/reference/document-types")
@Tag(name = "Reference Document Types", description = "Document type master APIs")
public class DocumentTypesController {

    private final ReferenceControllerService service;

    public DocumentTypesController(@Qualifier("documentTypesService") ReferenceControllerService service) {
        this.service = service;
    }

    @PostMapping
    public Mono<DocumentTypesViewDto> create(@Valid @RequestBody DocumentTypesUpsertRequestDto request) {
        return service.create(request.toReferenceRequest()).map(ReferenceViewDtoMapper::toDocumentTypes);
    }

    @PutMapping("/{id}")
    public Mono<DocumentTypesViewDto> update(@PathVariable("id") UUID id, @Valid @RequestBody DocumentTypesUpsertRequestDto request) {
        return service.update(id, request.toReferenceRequest()).map(ReferenceViewDtoMapper::toDocumentTypes);
    }

    @GetMapping("/{id}")
    public Mono<DocumentTypesViewDto> getById(@PathVariable("id") UUID id) {
        return service.getById(id).map(ReferenceViewDtoMapper::toDocumentTypes);
    }

    @GetMapping
    public Mono<PagedResult<DocumentTypesViewDto>> list(
        ...
        return service.list(new ReferenceSearchQuery(q, active, page, size, sort, skillCategoryId, all))
                .map(result -> new PagedResult<>(
                        result.items().stream().map(ReferenceViewDtoMapper::toDocumentTypes).toList(),
                        result.page(),
                        result.size(),
