...
@RestController
@Validated
@RequestMapping("/api/masters/reference/education-levels")
@Tag(name = "Reference Education Levels", description = "Education level master APIs")
public class EducationLevelsController {

    private final ReferenceControllerService service;

    public EducationLevelsController(@Qualifier("educationLevelsService") ReferenceControllerService service) {
        this.service = service;
    }

    @PostMapping
    public Mono<EducationLevelsViewDto> create(@Valid @RequestBody EducationLevelsUpsertRequestDto request) {
        return service.create(request.toReferenceRequest()).map(ReferenceViewDtoMapper::toEducationLevels);
    }

    @PutMapping("/{id}")
    public Mono<EducationLevelsViewDto> update(@PathVariable("id") UUID id, @Valid @RequestBody EducationLevelsUpsertRequestDto request) {
        return service.update(id, request.toReferenceRequest()).map(ReferenceViewDtoMapper::toEducationLevels);
    }

    @GetMapping("/{id}")
    public Mono<EducationLevelsViewDto> getById(@PathVariable("id") UUID id) {
        return service.getById(id).map(ReferenceViewDtoMapper::toEducationLevels);
    }

    @GetMapping
    public Mono<PagedResult<EducationLevelsViewDto>> list(
        ...
        return service.list(new ReferenceSearchQuery(q, active, page, size, sort, skillCategoryId, all))
                .map(result -> new PagedResult<>(
                        result.items().stream().map(ReferenceViewDtoMapper::toEducationLevels).toList(),
                        result.page(),
                        result.size(),
