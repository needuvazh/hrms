...
@RestController
@Validated
@RequestMapping("/api/masters/reference/genders")
@Tag(name = "Reference Genders", description = "Gender master APIs")
public class GendersController {

    private final ReferenceControllerService service;

    public GendersController(@Qualifier("gendersService") ReferenceControllerService service) {
        this.service = service;
    }

    @PostMapping
    public Mono<GendersViewDto> create(@Valid @RequestBody GendersUpsertRequestDto request) {
        return service.create(request.toReferenceRequest()).map(ReferenceViewDtoMapper::toGenders);
    }

    @PutMapping("/{id}")
    public Mono<GendersViewDto> update(@PathVariable("id") UUID id, @Valid @RequestBody GendersUpsertRequestDto request) {
        return service.update(id, request.toReferenceRequest()).map(ReferenceViewDtoMapper::toGenders);
    }

    @GetMapping("/{id}")
    public Mono<GendersViewDto> getById(@PathVariable("id") UUID id) {
        return service.getById(id).map(ReferenceViewDtoMapper::toGenders);
    }

    @GetMapping
    public Mono<PagedResult<GendersViewDto>> list(
        ...
        return service.list(new ReferenceSearchQuery(q, active, page, size, sort, skillCategoryId, all))
                .map(result -> new PagedResult<>(
                        result.items().stream().map(ReferenceViewDtoMapper::toGenders).toList(),
                        result.page(),
                        result.size(),
