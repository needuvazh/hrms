...
@RestController
@Validated
@RequestMapping("/api/masters/reference/languages")
@Tag(name = "Reference Languages", description = "Language master APIs")
public class LanguagesController {

    private final ReferenceControllerService service;

    public LanguagesController(@Qualifier("languagesService") ReferenceControllerService service) {
        this.service = service;
    }

    @PostMapping
    public Mono<LanguagesViewDto> create(@Valid @RequestBody LanguagesUpsertRequestDto request) {
        return service.create(request.toReferenceRequest()).map(ReferenceViewDtoMapper::toLanguages);
    }

    @PutMapping("/{id}")
    public Mono<LanguagesViewDto> update(@PathVariable("id") UUID id, @Valid @RequestBody LanguagesUpsertRequestDto request) {
        return service.update(id, request.toReferenceRequest()).map(ReferenceViewDtoMapper::toLanguages);
    }

    @PutMapping
    public Mono<LanguagesViewDto> updateByQueryId(
            @RequestParam("id") UUID id,
            @Valid @RequestBody LanguagesUpsertRequestDto request
    ) {
        return service.update(id, request.toReferenceRequest()).map(ReferenceViewDtoMapper::toLanguages);
    }

    @GetMapping("/{id}")
    public Mono<LanguagesViewDto> getById(@PathVariable("id") UUID id) {
        return service.getById(id).map(ReferenceViewDtoMapper::toLanguages);
    }

    @GetMapping
    public Mono<PagedResult<LanguagesViewDto>> list(
        ...
        return service.list(new ReferenceSearchQuery(q, active, page, size, sort, skillCategoryId, all))
                .map(result -> new PagedResult<>(
                        result.items().stream().map(ReferenceViewDtoMapper::toLanguages).toList(),
                        result.page(),
                        result.size(),
