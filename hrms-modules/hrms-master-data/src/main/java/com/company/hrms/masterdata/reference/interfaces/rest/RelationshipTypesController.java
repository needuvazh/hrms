...
@RestController
@Validated
@RequestMapping("/api/masters/reference/relationship-types")
@Tag(name = "Reference Relationship Types", description = "Relationship type master APIs")
public class RelationshipTypesController {

    private final ReferenceControllerService service;

    public RelationshipTypesController(@Qualifier("relationshipTypesService") ReferenceControllerService service) {
        this.service = service;
    }

    @PostMapping
    public Mono<RelationshipTypesViewDto> create(@Valid @RequestBody RelationshipTypesUpsertRequestDto request) {
        return service.create(request.toReferenceRequest()).map(ReferenceViewDtoMapper::toRelationshipTypes);
    }

    @PutMapping("/{id}")
    public Mono<RelationshipTypesViewDto> update(@PathVariable("id") UUID id, @Valid @RequestBody RelationshipTypesUpsertRequestDto request) {
        return service.update(id, request.toReferenceRequest()).map(ReferenceViewDtoMapper::toRelationshipTypes);
    }

    @GetMapping("/{id}")
    public Mono<RelationshipTypesViewDto> getById(@PathVariable("id") UUID id) {
        return service.getById(id).map(ReferenceViewDtoMapper::toRelationshipTypes);
    }

    @GetMapping
    public Mono<PagedResult<RelationshipTypesViewDto>> list(
        ...
        return service.list(new ReferenceSearchQuery(q, active, page, size, sort, skillCategoryId, all))
                .map(result -> new PagedResult<>(
                        result.items().stream().map(ReferenceViewDtoMapper::toRelationshipTypes).toList(),
                        result.page(),
                        result.size(),
