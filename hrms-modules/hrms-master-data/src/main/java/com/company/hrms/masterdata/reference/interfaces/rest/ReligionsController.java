...
@RestController
@Validated
@RequestMapping("/api/masters/reference/religions")
@Tag(name = "Reference Religions", description = "Religion master APIs")
public class ReligionsController {

    private final ReferenceControllerService service;

    public ReligionsController(@Qualifier("religionsService") ReferenceControllerService service) {
        this.service = service;
    }

    @PostMapping
    public Mono<ReligionsViewDto> create(@Valid @RequestBody ReligionsUpsertRequestDto request) {
        return service.create(request.toReferenceRequest()).map(ReferenceViewDtoMapper::toReligions);
    }

    @PutMapping("/{id}")
    public Mono<ReligionsViewDto> update(@PathVariable("id") UUID id, @Valid @RequestBody ReligionsUpsertRequestDto request) {
        return service.update(id, request.toReferenceRequest()).map(ReferenceViewDtoMapper::toReligions);
    }

    @GetMapping("/{id}")
    public Mono<ReligionsViewDto> getById(@PathVariable("id") UUID id) {
        return service.getById(id).map(ReferenceViewDtoMapper::toReligions);
    }

    @GetMapping
    public Mono<PagedResult<ReligionsViewDto>> list(
        ...
        return service.list(new ReferenceSearchQuery(q, active, page, size, sort, skillCategoryId, all))
                .map(result -> new PagedResult<>(
                        result.items().stream().map(ReferenceViewDtoMapper::toReligions).toList(),
                        result.page(),
                        result.size(),
