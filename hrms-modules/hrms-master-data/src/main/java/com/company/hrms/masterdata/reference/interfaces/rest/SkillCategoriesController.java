...
@RestController
@Validated
@RequestMapping("/api/masters/reference/skill-categories")
@Tag(name = "Reference Skill Categories", description = "Skill category master APIs")
public class SkillCategoriesController {

    private final ReferenceControllerService service;

    public SkillCategoriesController(@Qualifier("skillCategoriesService") ReferenceControllerService service) {
        this.service = service;
    }

    @PostMapping
    public Mono<SkillCategoriesViewDto> create(@Valid @RequestBody SkillCategoriesUpsertRequestDto request) {
        return service.create(request.toReferenceRequest()).map(ReferenceViewDtoMapper::toSkillCategories);
    }

    @PutMapping("/{id}")
    public Mono<SkillCategoriesViewDto> update(@PathVariable("id") UUID id, @Valid @RequestBody SkillCategoriesUpsertRequestDto request) {
        return service.update(id, request.toReferenceRequest()).map(ReferenceViewDtoMapper::toSkillCategories);
    }

    @GetMapping("/{id}")
    public Mono<SkillCategoriesViewDto> getById(@PathVariable("id") UUID id) {
        return service.getById(id).map(ReferenceViewDtoMapper::toSkillCategories);
    }

    @GetMapping
    public Mono<PagedResult<SkillCategoriesViewDto>> list(
        ...
        return service.list(new ReferenceSearchQuery(q, active, page, size, sort, skillCategoryId, all))
                .map(result -> new PagedResult<>(
                        result.items().stream().map(ReferenceViewDtoMapper::toSkillCategories).toList(),
                        result.page(),
                        result.size(),
