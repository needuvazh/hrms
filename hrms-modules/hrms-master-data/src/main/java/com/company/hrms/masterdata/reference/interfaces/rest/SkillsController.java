...
@RestController
@Validated
@RequestMapping("/api/masters/reference/skills")
@Tag(name = "Reference Skills", description = "Skill master APIs")
public class SkillsController {

    private final ReferenceControllerService service;

    public SkillsController(@Qualifier("skillsService") ReferenceControllerService service) {
        this.service = service;
    }

    @PostMapping
    public Mono<SkillsViewDto> create(@Valid @RequestBody SkillsUpsertRequestDto request) {
        return service.create(request.toReferenceRequest()).map(ReferenceViewDtoMapper::toSkills);
    }

    @PutMapping("/{id}")
    public Mono<SkillsViewDto> update(@PathVariable("id") UUID id, @Valid @RequestBody SkillsUpsertRequestDto request) {
        return service.update(id, request.toReferenceRequest()).map(ReferenceViewDtoMapper::toSkills);
    }

    @GetMapping("/{id}")
    public Mono<SkillsViewDto> getById(@PathVariable("id") UUID id) {
        return service.getById(id).map(ReferenceViewDtoMapper::toSkills);
    }

    @GetMapping
    public Mono<PagedResult<SkillsViewDto>> list(
        ...
        return service.list(new ReferenceSearchQuery(q, active, page, size, sort, skillCategoryId, all))
                .map(result -> new PagedResult<>(
                        result.items().stream().map(ReferenceViewDtoMapper::toSkills).toList(),
                        result.page(),
                        result.size(),
