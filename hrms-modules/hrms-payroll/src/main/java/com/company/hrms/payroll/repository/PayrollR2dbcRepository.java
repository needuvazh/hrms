...
public class PayrollR2dbcRepository implements PayrollRepository {
    ...
    public Flux<EarningComponentDto> saveEarningComponents(String tenantId, Flux<EarningComponentDto> components) {
        return components.concatMap(component -> databaseClient.sql("""
                ...
                .map((row, metadata) -> new EarningComponentDto(
                        row.get("id", UUID.class),
    ...
    public Flux<DeductionComponentDto> saveDeductionComponents(String tenantId, Flux<DeductionComponentDto> components) {
        return components.concatMap(component -> databaseClient.sql("""
                ...
                .bind("amount", component.amount())
                .bind("createdAt", component.createdAt())
                .map((row, metadata) -> new DeductionComponentDto(
                        row.get("id", UUID.class),
                        row.get("payroll_employee_record_id", UUID.class),
                        row.get("component_code", String.class),
                        row.get("component_name", String.class),
    ...
    private PayrollPeriodDto mapPayrollPeriod(
            UUID id,
            String tenantId,
            String periodCode,
            LocalDate startDate,
    ...
    private PayrollRunDto mapPayrollRun(
            UUID id,
            String tenantId,
    ...
    private PayrollEmployeeRecordDto mapPayrollEmployeeRecord(
            UUID id,
            String tenantId,
            UUID payrollRunId,
            UUID employeeId,
        ...
        return new PayrollEmployeeRecordDto(
                id,
                tenantId,
                payrollRunId,
                employeeId,
