package com.company.hrms.wps.infrastructure.persistence;
...
public class WpsR2dbcRepository implements WpsRepository {
    ...
    public Mono<WpsEmployeeEntry> saveEntry(WpsEmployeeEntry entry) {
        return databaseClient.sql("""
                ...
                .map((row, metadata) -> new WpsEmployeeEntry(
                        row.get("id", UUID.class),
                        row.get("tenant_id", String.class),
    ...
    private WpsExportFile mapExportFile(
            UUID id,
            String tenantId,
            UUID wpsBatchId,
            String exportType,
